...
{-
*Demo> :t (except $ Right 42) >> lift (put 5)
(except $ Right 42) >> lift (put 5) :: (Num s, Monad m) => ExceptT e (StateT s m) ()

*Demo> test $ (except $ Right 42) >> lift (put 5)
(Right (),5)

*Demo> :t (except $ Right 42) >> lift (modify (+1))
(except $ Right 42) >> lift (modify (+1)) :: (Num s, Monad m) => ExceptT e (StateT s m) ()

*Demo> test $ (except $ Right 42) >> lift (modify (+1))
(Right (),4)

*Demo> test $ throwE "Error!" >> lift (modify (+1))
(Left "Error!",3)
--}
